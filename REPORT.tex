...
\begin{center}
  ...
  {\small Lluís Estapé \quad$\cdot$\quad Musicathon by Musixmatch Pro 2026 \quad$\cdot$\quad 2026-06-11}
\end{center}
\vspace{0.4em}
\hrule
\vspace{1.2em}
...
\vspace{1em}
\statusbox{\textbf{Estado (2026-06-11).} Núcleo \textbf{verificado offline}: separación local
(Demucs), motor de audio, detección de beats (129.2 BPM / 299 beats en la canción de prueba) y la
app con tracking de manos + HUD + letra, todo funcionando \textbf{sin ninguna API}. Las APIs de
partners quedan pendientes de activar la \emph{key} (aprox. kickoff, 15 jun).}
...
{\footnotesize\begin{verbatim}
  Webcam -> MediaPipe (2 manos) -> landmarks
    ...
    |                   apertura pulgar-menique-> reverb
    |-- beat-sync (tecla b)                   -> los cambios de gain saltan al siguiente beat
    |
  Letra .lrc -> linea actual sincronizada a la posicion de reproduccion (karaoke)
\end{verbatim}}
...
\begin{itemize}
  \item Reproduce en bucle N stems (mono, 44.1 kHz) en un callback de \textbf{sounddevice} (bloque 512).
  \item \textbf{Mezcla} $=$ suma de (stem $\times$ gain), con rampa de gain por bloque y \emph{headroom}, $\times$ master.
...
posición de reproducción del motor. Se dibuja centrada en la parte inferior de la pantalla.
...
las fuentes son \textbf{intercambiables}.
...
\begin{itemize}
  \item \textbf{Kickoff (15 jun):} activar keys $\to$ cambiar Demucs por \textbf{LALAL.AI}, letra real
        de \textbf{Musixmatch}, BPM/mood de \textbf{Cyanite}.
  \item \textbf{Stretch:} richsync palabra-por-palabra $\cdot$ export de la remezcla a WAV $\cdot$
        \emph{pinch} $=$ solo de un stem $\cdot$ volumen master con la distancia entre manos $\cdot$
        landing en Replit para ``best use of Replit''.
\end{itemize}
